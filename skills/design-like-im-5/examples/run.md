...
`run/capabilities.json` via `mise run run-scaffold`
...
`run/dependency-manifest.json` via `mise run run-scaffold`
...
`run/dtcg-route.json` via `mise run run-scaffold`
...
`run/rebuild-queue.json` via `mise run run-scaffold`
...
`run/render-plan.json` via `mise run run-scaffold`
...
`run/retrieval-manifest.json` via `mise run run-scaffold`
...
`run/run.json` via `mise run run-scaffold`

```text
Mise owner: mise run run-scaffold
{"context_routing_sha256":"686b3db1241fe7b5df45508eed2675e47428af777623903570e742c6e5bc03d8","intake":{"audience":"New account owners.","outcome":"Build a clear account setup flow.","platform":"Responsive web.","primary_tasks":["Create an account","Fix input errors"],"proof_threshold":"Each task works in wide and narrow views.","source_permissions":["public_web","local_files"]},"next":"source_meaning","rules":["EYE_FIRST","BRAIN_ONE_STEP","TOUCH_CLEAR","NO_COLOR_ONLY","KEEP_INPUT","MOTION_ALT","WHOLE_SERVICE","SOURCE_IDEA","ADAPT_SPACE","STATE_OPEN","HUMAN_SWEEP","HARM_CHECKS","ONE_CHECK_ONE_MEANING"],"rules_selected":false,"run_id":"48843a797566d628","status":"READY","version":"1.0.0"}
```

`run/source-queue.json` via `mise run run-scaffold`
...
`run/state-matrix.json` via `mise run run-scaffold`
...
`run/viewport-matrix.json` via `mise run run-scaffold`
