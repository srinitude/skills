...
BLOCK_NAMES = ["If", "For", "While", "With", "Try", "TryStar", "Match",
               ...
               "ClassDef"]
BLOCKS = tuple(getattr(ast, name) for name in BLOCK_NAMES if hasattr(ast, name))
...
def check_construct(node, lines, path, problems):
    ...
    if size > MAX_CONSTRUCT:
        problems.append(f"{path}:{node.lineno}: {node.name} has {size} lines of code; cap is {MAX_CONSTRUCT}")
    if isinstance(node, FUNCS) and block_depth(node) > MAX_DEPTH:
        problems.append(f"{path}:{node.lineno}: {node.name} nesting is {block_depth(node)}; cap is {MAX_DEPTH}")
...
def check_file(path, problems):
    ...
    elif loc(text.splitlines()) > MAX_FILE:
        problems.append(f"{path}: {loc(text.splitlines())} lines of code; cap is 200")
