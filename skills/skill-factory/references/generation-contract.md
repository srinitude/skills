...
Implement the body's exact typed `mise run agentic-request` contract: digest-bound completed use case/outcome, prompt, skill dependencies and primitives with domain contribution/relevance/proof; verified UTF-8 snapshots in `use_case.text`/`skills[].text`/`context[].text`; file/stdin inputs; caller-owned runner/argument authority outside request data; no shell interpolation; structure/term/file/digest/exit/output checks followed by assigned semantic judgment. Never infer relevance, actual use or adequacy from checks.
