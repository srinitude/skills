...
def _TestRegistryStandardization_invoke(self, root, profile_path, apply=False):
    args = [root, "--profile", profile_path]
    return reviewed_standardize(*args, *(["--scope", "user", "--apply"] if apply else []))

def _TestRegistryStandardization_assert_package_files(self, root):
...
def _TestRegistryStandardization_assert_package_graph(self, root):
...
def _TestRegistryStandardization_test_plan_makes_no_writes(self):
...
def _TestRegistryStandardization_test_apply_refreshes_the_recursive_generation_contract(self):
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "clock-anchor"
        write_target(root)
...
def _TestRegistryStandardization_test_apply_refreshes_the_placeholder_gate(self):
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "clock-anchor"
        write_target(root)
...
def _TestRegistryStandardization_test_apply_updates_graph_and_preserves_source_inventory_assertion(self):
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "clock-anchor"
        write_target(root)
...
def _TestRegistryStandardization_test_refuses_profile_or_target_mismatch(self):
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "clock-anchor"
        write_target(root)
        data = profile()
...
def _TestRegistryStandardization_test_apply_adds_domain_owners_and_is_idempotent(self):
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "clock-anchor"
        write_target(root)
        profile_path = Path(temp) / "profile.json"
...
class TestRegistryStandardization(unittest.TestCase):
    invoke = _TestRegistryStandardization_invoke
    test_plan_makes_no_writes = _TestRegistryStandardization_test_plan_makes_no_writes
    test_apply_adds_domain_owners_and_is_idempotent = _TestRegistryStandardization_test_apply_adds_domain_owners_and_is_idempotent
    assert_package_files = _TestRegistryStandardization_assert_package_files
    assert_package_graph = _TestRegistryStandardization_assert_package_graph
    test_apply_refreshes_the_recursive_generation_contract = _TestRegistryStandardization_test_apply_refreshes_the_recursive_generation_contract
    test_apply_refreshes_the_placeholder_gate = _TestRegistryStandardization_test_apply_refreshes_the_placeholder_gate
    test_apply_updates_graph_and_preserves_source_inventory_assertion = _TestRegistryStandardization_test_apply_updates_graph_and_preserves_source_inventory_assertion
    test_refuses_profile_or_target_mismatch = _TestRegistryStandardization_test_refuses_profile_or_target_mismatch
