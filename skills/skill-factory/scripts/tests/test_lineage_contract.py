...
def _TestLineageContract_test_help_documents_usage_and_exit_codes(self):
...
def _TestLineageContract_test_refreshed_lineage_passes(self):
    refreshed = self.invoke()
    self.assertEqual(refreshed.returncode, 0, refreshed.stdout + refreshed.stderr)
    result = run('check_lineage.py', self.root)
    self.assertEqual(result.returncode, 0, result.stdout + result.stderr)

def _TestLineageContract_test_stale_public_version_fails(self):
...
def _TestLineageContract_test_generation_contract_uses_portable_name_grammar(self):
    text = (SKILL_DIR / 'references/generation-contract.md').read_text()
    self.assertIn('^[a-z0-9]+(?:-[a-z0-9]+)*$', text)
    self.assertNotIn('[a-z0-9._-]*', text)

def _TestLineageContract_test_runtime_trees_are_excluded_from_lineage(self):
...
def _TestLineageContract_test_runtime_symlinks_are_excluded_but_owned_links_cannot_change_lineage(self):
...
def _TestLineageContract_test_lineage_rejects_external_file_symlinks(self):
...
def _TestLineageContract_test_lineage_rejects_a_symlinked_skill_root(self):
...
class TestLineageContract(unittest.TestCase):
    ...
    test_help_documents_usage_and_exit_codes = _TestLineageContract_test_help_documents_usage_and_exit_codes
    test_refreshed_lineage_passes = _TestLineageContract_test_refreshed_lineage_passes
    test_stale_public_version_fails = _TestLineageContract_test_stale_public_version_fails
    test_generation_contract_uses_portable_name_grammar = _TestLineageContract_test_generation_contract_uses_portable_name_grammar
    test_runtime_trees_are_excluded_from_lineage = _TestLineageContract_test_runtime_trees_are_excluded_from_lineage
    test_runtime_symlinks_are_excluded_but_owned_links_cannot_change_lineage = _TestLineageContract_test_runtime_symlinks_are_excluded_but_owned_links_cannot_change_lineage
    test_lineage_rejects_external_file_symlinks = _TestLineageContract_test_lineage_rejects_external_file_symlinks
    test_lineage_rejects_a_symlinked_skill_root = _TestLineageContract_test_lineage_rejects_a_symlinked_skill_root
