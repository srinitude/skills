...
def _TestInitialContext_setUp(self):
...
def _TestInitialContext_invoke(self, code="import json,sys; print(json.dumps(json.load(sys.stdin)['context']))"):
...
def _TestInitialContext_assert_blocked(self, result):
    self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
    self.assertIn("context", result.stderr.lower())
    self.assertNotIn("Traceback", result.stderr)
    self.assertEqual(result.stdout, "")

def _TestInitialContext_test_current_package_body_is_automatic_and_precedes_requested_inputs(self):
    self.payload["skills"] = []
...
def _TestInitialContext_test_full_resources_reach_runner_in_dependency_order(self):
    result = self.invoke()
...
def _TestInitialContext_test_audience_failure_blocks_runner_and_valid_enum_recovers(self):
...
def _TestInitialContext_test_direct_dispatch_cannot_bypass_required_context(self):
    self.invoke()
...
def _TestInitialContext_test_missing_declaration_blocks_before_runner(self):
    self.data.pop("initial_context")
    self.assert_blocked(self.invoke("print('RUNNER_STARTED')"))

def _TestInitialContext_test_missing_duplicate_and_extra_inputs_block_before_runner(self):
...
def _TestInitialContext_test_stale_and_wrong_resource_bindings_block_then_recover(self):
...
def _TestInitialContext_test_invalid_reading_graph_blocks_before_runner(self):
...
def _TestInitialContext_test_ledger_and_nonempty_utf8_contents_are_required(self):
...
def _TestInitialContext_test_invocation_binding_captures_selected_ledger_and_keeps_package_pins(self):
...
def _TestInitialContext_test_only_contract_can_select_invocation_binding(self):
    original = json.dumps(self.data["initial_context"])
...
class TestInitialContext(unittest.TestCase):
    setUp = _TestInitialContext_setUp
    invoke = _TestInitialContext_invoke
    assert_blocked = _TestInitialContext_assert_blocked
    test_current_package_body_is_automatic_and_precedes_requested_inputs = _TestInitialContext_test_current_package_body_is_automatic_and_precedes_requested_inputs
    test_full_resources_reach_runner_in_dependency_order = _TestInitialContext_test_full_resources_reach_runner_in_dependency_order
    test_audience_failure_blocks_runner_and_valid_enum_recovers = _TestInitialContext_test_audience_failure_blocks_runner_and_valid_enum_recovers
    test_missing_declaration_blocks_before_runner = _TestInitialContext_test_missing_declaration_blocks_before_runner
    test_missing_duplicate_and_extra_inputs_block_before_runner = _TestInitialContext_test_missing_duplicate_and_extra_inputs_block_before_runner
    test_stale_and_wrong_resource_bindings_block_then_recover = _TestInitialContext_test_stale_and_wrong_resource_bindings_block_then_recover
    test_invalid_reading_graph_blocks_before_runner = _TestInitialContext_test_invalid_reading_graph_blocks_before_runner
    test_direct_dispatch_cannot_bypass_required_context = _TestInitialContext_test_direct_dispatch_cannot_bypass_required_context
    test_ledger_and_nonempty_utf8_contents_are_required = _TestInitialContext_test_ledger_and_nonempty_utf8_contents_are_required
    test_invocation_binding_captures_selected_ledger_and_keeps_package_pins = _TestInitialContext_test_invocation_binding_captures_selected_ledger_and_keeps_package_pins
    test_only_contract_can_select_invocation_binding = _TestInitialContext_test_only_contract_can_select_invocation_binding
