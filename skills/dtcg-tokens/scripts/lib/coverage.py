...
from .dtcg_resolution import ResolutionError, expand_document, resolve_token
from .dtcg_values import TYPES
...
def token_state(document):
    expanded, errors = expand_document(document)
    state = {"tokens": {}, "pointer_tokens": {}, "document": expanded, "reference_count": 0, "errors": errors}
...
def inventory(document):
    ...
    for path in sorted(state["tokens"]):
        ...
        try:
            resolved = resolve_token(state, path)
        except ResolutionError as error:
            state["errors"].append(f"{path}: {error}")
            resolved = None
        records.append({"path": path, "type": resolved_type(state, graph, path), "source_value": value, "resolved_value": resolved, "alias_targets": graph.get(path, []), "specimen_id": f"token-{len(records) + 1:04d}"})
...
def variant_groups(evidence, errors):
    ...
    for group in space.get("groups", []):
        ...
        for combo in itertools.product(*values):
            ...
            cells.append({"id": key, "values": list(combo), "status": "not_applicable" if exclusion else "expected", "reason": exclusion.get("reason") if exclusion else None})
        groups.append({"id": group["id"], "axes": group.get("axes", []), "expected": len(cells), "excluded": sum(item["status"] == "not_applicable" for item in cells), "cells": cells})
...
def analyze_coverage(tokens, evidence):
    ...
    types = [{"type": kind, "supplied": sum(item["type"] == kind for item in records), "expected_specimens": sum(item["type"] == kind for item in records), "status": "present" if any(item["type"] == kind for item in records) else "not_present"} for kind in sorted(TYPES)]
    composites = [part for record in records for part in composite_parts(record)]
    stress = stress_cells(records)
    totals = {"tokens_expected": len(records), "aliases": len(alias_edges(records)), "composite_members": len(composites), "stress_cells": len(stress), "variant_cells": sum(len(item["cells"]) for item in groups), "unexplained_skips": len(errors)}
    manifest = {"token_paths": records, "alias_edges": alias_edges(records), "type_coverage": types, "composite_members": composites, "stress_groups": stress, "variant_groups": groups, "totals": totals, "status": "planned" if not errors else "fail"}
