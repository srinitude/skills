...
4. Parse every uncompressed YAML prompt shard; verify manifest file hashes, prompt count, path order and uniqueness, exact UTF-8 byte counts, and every prompt SHA-256; reject any compressed tar or encoded prompt index.
5. Explicit tool triggers and fallbacks, including `image_generate`, `vision_analyze`, Flora `search_docs`, and Flora `execute`.
