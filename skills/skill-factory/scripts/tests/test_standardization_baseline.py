...
def _TestBaselineRecovery_commit(self, repo):
    subprocess.run(["git", "init", "-q", str(repo)], check=True)
    subprocess.run(["git", "-C", str(repo), "add", "."], check=True)
    subprocess.run(["git", "-C", str(repo), "-c", "user.name=Test",
                    "-c", "user.email=test@example.com", "commit", "-qm", "base"], check=True)

def _TestBaselineRecovery_test_restores_markdown_and_mapping_without_touching_code(self):
...
class TestBaselineRecovery(unittest.TestCase):
    test_restores_markdown_and_mapping_without_touching_code = _TestBaselineRecovery_test_restores_markdown_and_mapping_without_touching_code
    commit = _TestBaselineRecovery_commit
