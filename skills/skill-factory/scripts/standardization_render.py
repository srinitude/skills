...
from standardization_rules import BODY_POLICIES, body_policies, body_policy, route_policies, route_operations
...
def render(root, original, profile, scope, factory, sources, stamp):
    ...
    specs = route_policies(files, factory, profile['skill'])
    tasks = tomllib.loads(files['mise.toml'].decode('utf-8'))['tasks']
    contract_files(files, tasks, profile)
    asset_files(files, profile, tasks, stamp)
    route_operations(files, specs, profile)
