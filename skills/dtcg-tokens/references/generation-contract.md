...
No file names an agent product, an agent harness, or a company that ships one. The generic word `model` is permitted only in the vision execution gate that requires a strong vision-capable model or delegates all judgment work to one. Elsewhere use executor and describe the required action: run in a terminal, read the file, search the tree. Skills stay portable across executors, so nothing may assume host state beyond declared needs.
