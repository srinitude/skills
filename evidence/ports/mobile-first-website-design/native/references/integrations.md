...
Use deferred tools through `tool_search`, `tool_describe`, and `tool_call`:

- `mcp__mobbin__search_sections` for page sections
- `mcp__mobbin__search_screens` for single states
- `mcp__mobbin__search_flows` for multi-step behavior

Use fixed result limits. Inspect returned pixels with `vision_analyze`. Fallback: Refero, Lazyweb, then direct first-party web.
...
- Fallback: `image_generate`, ComfyUI, or an approved frozen local asset.
