...
def _TestRelatedOwnerWrite_setUp(self):
...
def _TestRelatedOwnerWrite_invoke(self, request=None, **kwargs):
    return write_file(request or self.request, self.root, target_root=self.folder, **kwargs)

def _TestRelatedOwnerWrite_test_related_root_must_be_a_canonical_ancestor_and_is_not_request_authority(self):
...
def _TestRelatedOwnerWrite_test_related_target_uses_installed_body_and_preserves_mode(self):
    result = self.invoke()
...
def _TestRelatedOwnerWrite_test_related_target_rejects_body_modes_paths_and_overlapping_inputs(self):
...
def _TestRelatedOwnerWrite_test_post_failure_restores_only_current_related_effect_and_recovers(self):
    def predicate():
...
def _TestRelatedOwnerWrite_test_stale_body_review_and_related_link_reject_before_effect(self):
...
class TestRelatedOwnerWrite(unittest.TestCase):
    setUp = _TestRelatedOwnerWrite_setUp
    invoke = _TestRelatedOwnerWrite_invoke
    test_related_target_uses_installed_body_and_preserves_mode = _TestRelatedOwnerWrite_test_related_target_uses_installed_body_and_preserves_mode
    test_related_target_rejects_body_modes_paths_and_overlapping_inputs = _TestRelatedOwnerWrite_test_related_target_rejects_body_modes_paths_and_overlapping_inputs
    test_related_root_must_be_a_canonical_ancestor_and_is_not_request_authority = _TestRelatedOwnerWrite_test_related_root_must_be_a_canonical_ancestor_and_is_not_request_authority
    test_post_failure_restores_only_current_related_effect_and_recovers = _TestRelatedOwnerWrite_test_post_failure_restores_only_current_related_effect_and_recovers
    test_stale_body_review_and_related_link_reject_before_effect = _TestRelatedOwnerWrite_test_stale_body_review_and_related_link_reject_before_effect
