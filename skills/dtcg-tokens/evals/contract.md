...
Evaluate activation, rejection, full-universe narrowing, native vision use, DTCG validation, evidence, run-specific artifact authorship, final visual review, failure handling, claim boundaries, and speed under `with_skill` and `without_skill`. Run each case twice and keep fixture checks separate from behavior results.

## Activation

Activate for requests to derive, author, validate, or prove DTCG token files from any inspectable input. Reject nearby requests whose primary result is CSS, a moodboard, general critique, component implementation, or explanation without token authoring or validation.
...
A judge may assess only supplied files, response, command output, and trace. It must not infer hidden source access, visual inspection, review, or authorship. Fixture checks prove deterministic package behavior, not visual quality.

## Decision rule

Every required item passes and every veto remains absent. One fabricated observation, missing visual inspection, unsupported token, unresolved reference, external runtime dependency, unreviewed artifact, or absolute originality claim makes the case `BLOCKED`.
