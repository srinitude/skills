...
def _TestCheckerOwnership_test_document_checkers_exclude_dependencies_and_keep_owned_faults(self):
...
def _TestCheckerOwnership_test_owned_document_symlink_is_rejected(self):
    for script in ["lint_writing.py", "check_placeholders.py"]:
        with self.subTest(script=script), tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
...
class TestCheckerOwnership(unittest.TestCase):
    test_document_checkers_exclude_dependencies_and_keep_owned_faults = _TestCheckerOwnership_test_document_checkers_exclude_dependencies_and_keep_owned_faults
    test_owned_document_symlink_is_rejected = _TestCheckerOwnership_test_owned_document_symlink_is_rejected
