...
def _TestStandardizationWorkflow_setUp(self):
...
def _TestStandardizationWorkflow_invoke(self, *options, public=False, args=None, expected=0):
...
def _TestStandardizationWorkflow_snapshot(self):
    return {p.relative_to(self.root).as_posix(): (p.read_bytes(), p.stat().st_mode & 0o777)
            for p in self.root.rglob('*') if p.is_file()}

def _TestStandardizationWorkflow_review(self, planned):
...
def _TestStandardizationWorkflow_report(self, *options, **kwargs):
...
def _TestStandardizationWorkflow_check_output(self, planned):
...
def _TestStandardizationWorkflow_test_usage_and_missing_target_reject_before_state_effects(self):
...
def _TestStandardizationWorkflow_test_database_alias_and_target_state_reject_without_file_effects(self):
...
def _TestStandardizationWorkflow_test_public_help_and_transient_plan_expose_the_real_workflow_without_target_effects(self):
...
def _TestStandardizationWorkflow_test_saved_plan_applies_through_a_new_transient_workflow(self):
    planned = self.report()
...
def _TestStandardizationWorkflow_test_separate_process_resumes_exact_run_then_rejects_completed_replay(self):
...
def _TestStandardizationWorkflow_test_explicit_rejection_closes_run_without_apply_and_fresh_review_recovers(self):
    before = self.snapshot()
...
class TestStandardizationWorkflow(unittest.TestCase):
    setUp = _TestStandardizationWorkflow_setUp
    invoke = _TestStandardizationWorkflow_invoke
    report = _TestStandardizationWorkflow_report
    snapshot = _TestStandardizationWorkflow_snapshot
    review = _TestStandardizationWorkflow_review
    check_output = _TestStandardizationWorkflow_check_output
    test_public_help_and_transient_plan_expose_the_real_workflow_without_target_effects = _TestStandardizationWorkflow_test_public_help_and_transient_plan_expose_the_real_workflow_without_target_effects
    test_usage_and_missing_target_reject_before_state_effects = _TestStandardizationWorkflow_test_usage_and_missing_target_reject_before_state_effects
    test_saved_plan_applies_through_a_new_transient_workflow = _TestStandardizationWorkflow_test_saved_plan_applies_through_a_new_transient_workflow
    test_separate_process_resumes_exact_run_then_rejects_completed_replay = _TestStandardizationWorkflow_test_separate_process_resumes_exact_run_then_rejects_completed_replay
    test_database_alias_and_target_state_reject_without_file_effects = _TestStandardizationWorkflow_test_database_alias_and_target_state_reject_without_file_effects
    test_explicit_rejection_closes_run_without_apply_and_fresh_review_recovers = _TestStandardizationWorkflow_test_explicit_rejection_closes_run_without_apply_and_fresh_review_recovers
