...
class TestStandardizationUsage(unittest.TestCase):
    def python(self, code, *args, expected=0):
    ...
    def usage(self, *args, stdin=None):
        result = subprocess.run(['usage', *args], input=stdin, capture_output=True,
                                text=True, timeout=30)
        self.assertEqual(result.returncode, 0, result.stderr)
        return result.stdout

    def test_current_export_retains_native_help_choices_and_workflow_notes(self):
    ...
    def test_native_and_usage_bindings_match_and_invalid_arguments_reject(self):
    ...
    def workflow(self, factory, args, expected):
    ...
    def test_workflow_rejects_stale_projection_before_state_and_current_bytes_recover(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
    ...
    def test_missing_or_changed_projection_rejects_and_exact_restoration_recovers(self):
        with tempfile.TemporaryDirectory() as directory:
