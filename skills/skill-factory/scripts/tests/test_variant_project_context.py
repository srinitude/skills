...
class TestVariantProjectContext(unittest.TestCase):
    def test_project_destination_inside_declared_project_accepts_without_context_drift(self):
    ...
    def test_missing_project_review_fails_before_promotion(self):
        with tempfile.TemporaryDirectory() as temp:
            base = Path(temp)
            source = package(base / "source" / "demo-skill", "user")
            candidate = package(base / "stage" / "atlas-inventory", "project")
    ...
    def test_edited_plan_cannot_place_variant_inside_source(self):
        with tempfile.TemporaryDirectory() as temp:
            base = Path(temp)
