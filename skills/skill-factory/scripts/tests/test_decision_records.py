...
def _TestDecisionRecords_test_help_documents_usage_and_exit_codes(self):
    result = run("check_decision_records.py", "--help")
    self.assertEqual(result.returncode, 0)
    self.assertIn("exit code", result.stdout.lower())

def _TestDecisionRecords_test_deterministic_and_model_owned_decisions_pass(self):
...
def _TestDecisionRecords_test_missing_motivation_fails(self):
    item = record()
    del item["motivation"]
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "release-notes"
...
def _TestDecisionRecords_test_generic_reason_without_domain_term_fails(self):
    item = record()
...
def _TestDecisionRecords_test_domain_label_cannot_decorate_generic_decision(self):
    item = record()
    item["why_this_path"] = (
...
def _TestDecisionRecords_test_decision_kind_controls_owner(self):
...
def _TestDecisionRecords_test_domain_term_must_not_match_inside_an_unrelated_word(self):
    item = record()
...
class TestDecisionRecords(unittest.TestCase):
    test_help_documents_usage_and_exit_codes = _TestDecisionRecords_test_help_documents_usage_and_exit_codes
    test_deterministic_and_model_owned_decisions_pass = _TestDecisionRecords_test_deterministic_and_model_owned_decisions_pass
    test_missing_motivation_fails = _TestDecisionRecords_test_missing_motivation_fails
    test_generic_reason_without_domain_term_fails = _TestDecisionRecords_test_generic_reason_without_domain_term_fails
    test_domain_label_cannot_decorate_generic_decision = _TestDecisionRecords_test_domain_label_cannot_decorate_generic_decision
    test_decision_kind_controls_owner = _TestDecisionRecords_test_decision_kind_controls_owner
    test_domain_term_must_not_match_inside_an_unrelated_word = _TestDecisionRecords_test_domain_term_must_not_match_inside_an_unrelated_word
