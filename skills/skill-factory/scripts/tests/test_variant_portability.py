...
def _TestVariantPortability_test_unrelated_metadata_cannot_be_lost_in_a_variant(self):
...
def _TestVariantPortability_test_private_source_content_is_excluded_and_source_can_be_unavailable(self):
    with tempfile.TemporaryDirectory() as temp:
        base = Path(temp)
        source = package(base / "source" / "demo-skill", "project")
...
def _TestVariantPortability_test_plan_and_unaccepted_review_artifacts_are_exact_and_collision_safe(self):
    with tempfile.TemporaryDirectory() as temp:
        base = Path(temp)
        source = package(base / "source" / "demo-skill", "project")
        candidate = package(base / "stage" / "portable-inventory", "user")
...
class TestVariantPortability(unittest.TestCase):
    test_unrelated_metadata_cannot_be_lost_in_a_variant = _TestVariantPortability_test_unrelated_metadata_cannot_be_lost_in_a_variant
    test_private_source_content_is_excluded_and_source_can_be_unavailable = _TestVariantPortability_test_private_source_content_is_excluded_and_source_can_be_unavailable
    test_plan_and_unaccepted_review_artifacts_are_exact_and_collision_safe = _TestVariantPortability_test_plan_and_unaccepted_review_artifacts_are_exact_and_collision_safe
