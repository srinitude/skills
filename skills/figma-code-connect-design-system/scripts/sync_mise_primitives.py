#!/usr/bin/env python3
"""Get the official Mise part list for this Figma skill."""
import argparse
import datetime
import hashlib
import json
...
def schema_bytes(version, schema_file):
    ...
    url = f"https://raw.githubusercontent.com/jdx/mise/v{version}/schema/mise.json"
    try:
        with urllib.request.urlopen(url, timeout=30) as response:
            return response.read()
    except (urllib.error.URLError, TimeoutError) as error:
        raise ValueError(f"cannot read the Mise form: {error}") from error
...
def schema_groups(schema):
    ...
    task_config = defs.get("task_config", {}).get("properties", {})
    return {"config": sorted(schema.get("properties", {})),
            "task": sorted(task), "task_config": sorted(task_config),
            "tool": sorted(tool)}


def catalog(version, raw):
    groups = schema_groups(json.loads(raw))
    missing = [name for name, values in groups.items() if not values]
    if missing:
        raise ValueError("missing Mise part group: " + ", ".join(missing))
    checked = datetime.datetime.now().astimezone().isoformat(timespec="seconds")
    return {"version": version, "source": SCHEMA_ID,
            "release_source": f"https://raw.githubusercontent.com/jdx/mise/v{version}/schema/mise.json",
            "task_source": TASK_SOURCE, "checked_at": checked,
...
def main():
    ...
    if args.check:
        print(f"Figma Mise list: {'old' if stale else 'current'}")
        return 1 if stale else 0
    atomic_write(path, expected)
    print(f"Figma Mise list: new {version}")
    return 0
