...
class TestVariantPublicationGuard(unittest.TestCase):
    ...
    def test_missing_ledger_review_rejects_before_publication(self):
    ...
    def test_preview_contains_every_future_file_without_publishing(self):
        before = inventory(self.source), inventory(self.candidate)
    ...
    def test_incomplete_review_and_mode_drift_reject_then_current_review_publishes(self):
        result = run('skill_variant.py', *self.args, '--preview')
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_source_drift_during_promotion_is_retained_and_publication_is_withdrawn(self):
        result = run('skill_variant.py', *self.args, '--preview')
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        publication = json.loads(result.stdout)
