...
These are the judgment half of workflow step 3. Run them on every prompt after `mise run check-prompt` has covered the deterministic half. Every finding, from scripts and from you, gets one of two dispositions: fixed in the rewrite, or listed as an open question.
