...
Owner and backlink: load [`../SKILL.md`](../SKILL.md) through `mise run evals`. This file owns the registry regression procedure for skill-factory. The parent owns runtime behavior.

## Frozen inputs

Freeze and hash these inputs before a behavioral run:

- the deployed `SKILL.md` and every referenced file under `references/`, loaded through `mise run evals`;
...
2. Permit no side effects except those authorized by the case, and route deterministic checks through their owning Mise tasks while retaining every authorized model capability the case requires.
