#!/usr/bin/env python3
"""Get the official Mise part list."""
import argparse
import datetime
import hashlib
import json
...
def catalog(version, raw):
    ...
    return {"version": version, "source": SCHEMA_ID,
            ...
            "schema_sha256": hashlib.sha256(raw).hexdigest(), "groups": groups}
...
def main():
    ...
    if args.check:
        print(f"design Mise list: {'old' if stale else 'current'}")
        return 1 if stale else 0
    atomic_write(path, expected)
    print(f"design Mise list: new {version}; use notes did not change")
    return 0
