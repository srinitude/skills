...
def _TestVariantBoundaries_test_label_only_generalization_fails_actual_second_project_behavior(self):
...
def _TestVariantBoundaries_test_hidden_source_dependency_and_missing_attribution_block_acceptance(self):
    with tempfile.TemporaryDirectory() as temp:
        base = Path(temp)
        source = package(base / "source" / "demo-skill", "project")
...
def _TestVariantBoundaries_test_explicit_in_place_scope_change_requires_and_passes_adaptation(self):
    with tempfile.TemporaryDirectory() as temp:
        base = Path(temp)
        source = package(base / "source" / "demo-skill", "project")
...
def _TestVariantBoundaries_test_source_change_after_plan_rejects_candidate_and_preserves_current_source(self):
    with tempfile.TemporaryDirectory() as temp:
        base = Path(temp)
        source = package(base / "source" / "demo-skill", "project")
        candidate = package(base / "stage" / "portable-inventory", "user")
...
class TestVariantBoundaries(unittest.TestCase):
    test_label_only_generalization_fails_actual_second_project_behavior = _TestVariantBoundaries_test_label_only_generalization_fails_actual_second_project_behavior
    test_hidden_source_dependency_and_missing_attribution_block_acceptance = _TestVariantBoundaries_test_hidden_source_dependency_and_missing_attribution_block_acceptance
    test_explicit_in_place_scope_change_requires_and_passes_adaptation = _TestVariantBoundaries_test_explicit_in_place_scope_change_requires_and_passes_adaptation
    test_source_change_after_plan_rejects_candidate_and_preserves_current_source = _TestVariantBoundaries_test_source_change_after_plan_rejects_candidate_and_preserves_current_source
