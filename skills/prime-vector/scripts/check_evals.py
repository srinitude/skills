...
def check_case(case, index, skill, problems):
    ...
    elif not all(nonempty(assertion) for assertion in assertions):
...
def check_cases(document, skill, minimum, problems):
    if not nonempty(document.get("skill_name")):
...
def check_queries(queries, minimum, problems):
    if not isinstance(queries, list) or len(queries) < minimum:
        problems.append(f"trigger-queries.json: needs at least {minimum} queries")
        return
    labels = set()
    for index, entry in enumerate(queries, start=1):
        if not isinstance(entry, dict):
            problems.append(f"trigger-queries.json entry {index}: must be an object")
            continue
        if not nonempty(entry.get("query")):
            problems.append(f"trigger-queries.json entry {index}: query must be a non-empty string")
        flag = entry.get("should_trigger")
        if not isinstance(flag, bool):
            problems.append(f"trigger-queries.json entry {index}: should_trigger must be true or false")
        labels.add(flag)
    if not {True, False} <= labels:
        problems.append("trigger-queries.json: needs positive and negative queries")
...
def main(argv=None):
    ...
    if not skill.is_dir():
        ...
        return 2
    problems = []
    document = load(skill / "evals" / "evals.json", problems)
    if document is not None:
        check_cases(document, skill, args.min_cases, problems)
