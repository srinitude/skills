...
4. `possibility-universe` expands every leaf in `assets/token-possibility-catalog.json`, every applicable input-intent branch, every review marker, and every invariant before exclusion. Use `mise run token-packet -- <args>` for this routed resource.
5. `narrow` loads `assets/exploration-strategy-catalog.json`, creates one candidate for every applicable strategy in fixed order, records all candidates, and applies the catalog's ordered gates. Each stage output is a subset of its input. At least three candidates from at least three strategies remain included, with at least one inversion or antithesis. Use `mise run token-packet -- <args>` for this routed resource.
...
10. `assemble` runs `mise run token-prove` to validate obligations and embed canonical data. The script does not choose layout, styling, copy, or verdict.
...
Stable machine-readable obligations are allowed. The candidate HTML contains each `data-proof-obligation` exactly once and the required assembly placeholders. `mise run token-prove` fills those placeholders and canonical records. It never creates or certifies the design.
...
Use `references/visual-review.md`. Keep objective defects, invariants, taste, originality, corpus uniqueness, and non-AI-slop as separate verdicts. Use `mise run token-packet -- <args>` for this routed resource.
...
Use the creative exception protocol in `assets/perceptual-motor-invariant-catalog.json`. A failed experimental use blocks a clean pass for that context, not continued generation, comparison, or learning from the branch. Use `mise run token-packet -- <args>` for this routed resource.

## Failures and recovery

Use one stable code from `assets/execution-step-contract.json`. Never invent an error code in a run record. Use `mise run token-packet -- <args>` for this routed resource.
