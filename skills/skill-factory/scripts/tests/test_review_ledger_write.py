...
def _TestLedgerWrite_setUp(self):
    ...
    self.data['dependency_traversal'] = {'workflow': [{'step': 1, 'action': 'Read', 'rule': 'Read every governing input.'}]}
    self.data['reusable_review_protocol'] = [ {'field': 'Contribution', 'review': 'Explain the actual change.'},
        {'field': 'Body decision', 'review': 'Keep core rules in the body.'}]
    self.data['semantic_model'] = {'body_hub': {'core': 'SKILL.md'}, 'mechanism_map': {'write': 'The declared file owner.'}}
    ...
    self.request.update(action='write-file', ledger=str(self.ledger), ledger_sha256=sha(self.ledger.read_bytes()),
        change={'path': 'result.bin', 'expected_sha256': None, 'new_file': {'path': str(self.prepared), 'sha256': sha(self.prepared.read_bytes())},
                'body_sha256': sha((self.root / 'SKILL.md').read_bytes()), 'reviewer': 'Test author; mechanical fixture only',
                'review': {'Contribution': 'Exercise exact binary file creation.', 'Body decision': 'The fixture body already declares the write condition.'}})
    self.request['initial_body_review'] = initial_body_review(self)

def _TestLedgerWrite_invoke(self, request=None):
    from review_ledger_write import write_file
    return write_file(request or self.request, self.root)

def _TestLedgerWrite_package(self):
    return {p.relative_to(self.root).as_posix(): (p.read_bytes(), p.stat().st_mode) for p in self.root.rglob('*') if p.is_file()}

def _TestLedgerWrite_native(self, request=None, root=True, public=False):
    path = self.folder / 'request.json'
    path.write_text(json.dumps(request or self.request))
    command = ['mise', 'run', '--force', '--task-cache', 'off', 'ledger', '--'] if public else ['node', str(ROOT / 'scripts/run_review_ledger.ts')]
    command.append(str(path))
    if root:
        command += ['--write-root', str(self.root)]
    return subprocess.run(command, cwd=ROOT, capture_output=True, text=True, timeout=60)

def _TestLedgerWrite_test_creation_replacement_and_repeated_stale_request_rejection(self):
    result = self.invoke()
...
def _TestLedgerWrite_test_stale_missing_and_incomplete_reviews_reject_without_mutation(self):
    changes = [lambda r: r.update(ledger_sha256='0'*64), lambda r: r['change'].update(body_sha256='0'*64),
               lambda r: r['change']['new_file'].update(sha256='0'*64), lambda r: r['expected_documents'].pop(),
               lambda r: r['original_source'].update(sha256='0'*64), lambda r: r['change']['review'].pop('Body decision'),
               lambda r: r['change']['review'].update(Contribution=''), lambda r: r['change'].update(reviewer=''),
...
def _TestLedgerWrite_test_missing_or_empty_body_rejects_before_effect_and_recovers(self):
...
def _TestLedgerWrite_test_paths_links_and_source_overlap_reject_and_valid_input_recovers(self):
    before = self.package()
...
def _TestLedgerWrite_test_package_lock_rejects_before_writing_and_recovers(self):
...
def _TestLedgerWrite_test_native_root_is_external_to_request_and_read_actions_cannot_write(self):
    before = self.package()
    self.assertEqual(self.native(root=False).returncode, 1)
    for operation in ['parse', 'view']:
        process = subprocess.run([sys.executable, str(ROOT / 'scripts/review_ledger.py'), operation, '--write-root', ''], input='{}', capture_output=True, text=True)
...
class TestLedgerWrite(unittest.TestCase):
    setUp = _TestLedgerWrite_setUp
    invoke = _TestLedgerWrite_invoke
    package = _TestLedgerWrite_package
    native = _TestLedgerWrite_native
    test_creation_replacement_and_repeated_stale_request_rejection = _TestLedgerWrite_test_creation_replacement_and_repeated_stale_request_rejection
    test_stale_missing_and_incomplete_reviews_reject_without_mutation = _TestLedgerWrite_test_stale_missing_and_incomplete_reviews_reject_without_mutation
    test_missing_or_empty_body_rejects_before_effect_and_recovers = _TestLedgerWrite_test_missing_or_empty_body_rejects_before_effect_and_recovers
    test_paths_links_and_source_overlap_reject_and_valid_input_recovers = _TestLedgerWrite_test_paths_links_and_source_overlap_reject_and_valid_input_recovers
    test_package_lock_rejects_before_writing_and_recovers = _TestLedgerWrite_test_package_lock_rejects_before_writing_and_recovers
    test_native_root_is_external_to_request_and_read_actions_cannot_write = _TestLedgerWrite_test_native_root_is_external_to_request_and_read_actions_cannot_write
