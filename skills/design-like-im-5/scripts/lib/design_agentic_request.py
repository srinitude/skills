"""Validate one typed product-design agentic request."""
import hashlib
import json
from pathlib import Path

from lib.design_policy_text import words

TRACE_FIELDS = ("domain_role", "outcome_contribution", "relevance",
                "expected_proof")


def digest(path):
    return hashlib.sha256(path.read_bytes()).hexdigest()


def uses_term(text, terms):
    normalized = " ".join(words(text))
    return any(" ".join(words(term)) in normalized for term in terms)


def checked_file(item, label, base):
    if not isinstance(item, dict):
        raise ValueError(f"{label} must be an object")
    raw, expected = item.get("path"), item.get("sha256")
    if not isinstance(raw, str) or not raw:
        raise ValueError(f"{label}.path must be nonempty text")
    if not isinstance(expected, str) or len(expected) != 64:
        raise ValueError(f"{label}.sha256 must be a full digest")
    path = Path(raw) if Path(raw).is_absolute() else base / raw
    if not path.is_file() or path.is_symlink():
        raise ValueError(f"{label}.path must be a regular file")
    if digest(path) != expected:
        raise ValueError(f"{label} digest mismatch")
    return path.resolve()


def trace(item, label, terms):
    if not isinstance(item, dict):
        raise ValueError(f"{label}.trace must be an object")
    result = {}
    for field in TRACE_FIELDS:
        value = item.get(field)
        if not isinstance(value, str) or len(value.strip()) < 20:
            raise ValueError(f"{label}.trace.{field} needs specific text")
        if not uses_term(value, terms):
            raise ValueError(f"{label}.trace.{field} needs a product design term")
        result[field] = value.strip()
    if len({value.casefold() for value in result.values()}) != len(result):
        raise ValueError(f"{label}.trace fields must not repeat one claim")
    return result


def use_case(item, base):
    path = checked_file(item, "use_case", base)
    data = json.loads(path.read_text(encoding="utf-8"))
    terms, outcome = data.get("domain_terms"), data.get("outcome")
    if path.name != "use-case-contract.json":
        raise ValueError("use_case.path must name use-case-contract.json")
    if not isinstance(terms, list) or len(terms) < 3:
        raise ValueError("use-case contract needs product design terms")
    if item.get("promised_outcome") != outcome:
        raise ValueError("promised outcome must match the product design contract")
    operations = data.get("task_graph", {}).get("public_operations", [])
    matches = [row for row in operations if row.get("task") == "agentic-request"]
    if len(matches) != 1 or "agentic-request" not in data["task_graph"]["tasks"]:
        raise ValueError("agentic-request needs one product design task and operation")
    return {"path": str(path), "sha256": digest(path), "skill": data["skill"],
            "promised_outcome": outcome, "domain_terms": terms}, terms


def prompt(item, base, terms):
    if not isinstance(item, dict):
        raise ValueError("prompt must be an object")
    choices = [key for key in ("text", "file") if key in item]
    if len(choices) != 1:
        raise ValueError("prompt needs exactly one of text or file")
    if choices[0] == "file":
        path = checked_file(
            {"path": item["file"], "sha256": item.get("sha256")},
            "prompt", base)
        value = path.read_text(encoding="utf-8")
    else:
        value = item["text"]
    if not isinstance(value, str) or not uses_term(value, terms):
        raise ValueError("prompt needs product design content")
    return value, trace(item.get("trace"), "prompt", terms)


def skills(items, base, terms):
    if not isinstance(items, list):
        raise ValueError("skills must be an array")
    result = []
    for index, item in enumerate(items):
        path = checked_file(item, f"skills.{index}", base)
        if path.name != "SKILL.md":
            raise ValueError(f"skills.{index}.path must name SKILL.md")
        result.append({"path": str(path), "sha256": digest(path),
                       "trace": trace(item.get("trace"), f"skills.{index}", terms)})
    return result


def primitives(items, terms):
    if not isinstance(items, list):
        raise ValueError("primitives must be an array")
    result = []
    for index, item in enumerate(items):
        if not isinstance(item, dict) or "configuration" not in item:
            raise ValueError(f"primitives.{index} is incomplete")
        name, kind = item.get("name"), item.get("kind")
        if not isinstance(kind, str) or not kind.strip():
            raise ValueError(f"primitives.{index}.kind needs text")
        if not isinstance(name, str) or not uses_term(name, terms):
            raise ValueError(f"primitives.{index}.name needs a product design term")
        result.append({"kind": kind, "name": name,
                       "configuration": item["configuration"],
                       "trace": trace(item.get("trace"), f"primitives.{index}", terms)})
    return result


def build_envelope(data, base):
    if not isinstance(data, dict) or data.get("version") != 1:
        raise ValueError("request must be a version 1 object")
    if "runner" in data:
        raise ValueError("request data cannot choose its runner")
    case, terms = use_case(data.get("use_case"), base)
    operation = data.get("operation")
    if not isinstance(operation, str) or not uses_term(operation, terms):
        raise ValueError("operation needs a product design term")
    text, prompt_trace = prompt(data.get("prompt"), base, terms)
    return {"version": 1, "operation": operation, "use_case": case,
            "prompt": text, "prompt_sha256": hashlib.sha256(text.encode()).hexdigest(),
            "prompt_trace": prompt_trace,
            "skills": skills(data.get("skills", []), base, terms),
            "primitives": primitives(data.get("primitives", []), terms)}
