...
def _TestVariantPlanning_test_missing_project_context_does_not_write(self):
...
def _TestVariantPlanning_test_plan_records_complete_source_and_project_context(self):
    with tempfile.TemporaryDirectory() as temp:
        scaffold(temp)
        source = Path(temp) / "demo-skill"
...
def _TestVariantPlanning_test_same_name_refuses_shadowing_and_existing_target_refuses_overwrite(self):
    with tempfile.TemporaryDirectory() as temp:
...
def _TestVariantPlanning_test_source_symlink_rejected_before_read(self):
    with tempfile.TemporaryDirectory() as temp:
        scaffold(temp)
...
class TestVariantPlanning(unittest.TestCase):
    test_missing_project_context_does_not_write = _TestVariantPlanning_test_missing_project_context_does_not_write
    test_plan_records_complete_source_and_project_context = _TestVariantPlanning_test_plan_records_complete_source_and_project_context
    test_same_name_refuses_shadowing_and_existing_target_refuses_overwrite = _TestVariantPlanning_test_same_name_refuses_shadowing_and_existing_target_refuses_overwrite
    test_source_symlink_rejected_before_read = _TestVariantPlanning_test_source_symlink_rejected_before_read
