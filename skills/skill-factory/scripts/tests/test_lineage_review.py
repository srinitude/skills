...
class TestLineageReview(unittest.TestCase):
    ...
    def test_plan_is_complete_and_read_only(self):
        before = self.package()
    ...
    def test_reviewed_creation_replacement_modes_and_stale_rejection(self):
        result = self.invoke()
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_wrong_target_tampered_metadata_and_stale_review_reject_then_recover(self):
    ...
    def test_package_and_request_drift_restore_lineage_then_current_inputs_recover(self):
    ...
    def test_cooperating_lock_blocks_effect_then_release_recovers(self):
        before = self.package()
    ...
    def test_false_or_null_effect_checks_cannot_succeed(self):
